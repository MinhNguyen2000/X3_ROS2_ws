...
class FaceFollowerNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("min_displacement", 0.50)        # (m) - min face movement before sending a new goal
        self.declare_parameter("max_displacement", 2.00)
        self.declare_parameter("min_time_between_goals", 5.0)   # (s) - min time between face goal updates
        self.declare_parameter("tf_timeout", 0.1)               # (s) - time waiting for a tf_lookup

        self.agent_name             = self.get_parameter("agent_name").value
        # TODO - dynamically set the odom frame using the user-specified odom frame name
        self.odom_frame             = self.get_parameter("odom_frame").value
        self.odom_frame             = f"{self.agent_name}_odom"
        self.robot_base_frame       = self.get_parameter("robot_base_frame").value
        self.goal_tolerance         = self.get_parameter("goal_tolerance").value
        self.min_displacement       = self.get_parameter("min_displacement").value
        self.max_displacement       = self.get_parameter("max_displacement").value
    ...
    def face_pose_callback(self, msg: PoseStamped):
        ...
        # DEBUG - print the coordinate of the detected face in odom frame
        ...
        if self._last_goal_odom is not None:
            ...
            if (displacement >= self.max_displacement 
                ...
                or time_elapsed <= self.min_time_between_goals
            ):
                return
    ...
    def _goal_result_callback(self, future):
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            self.get_logger().info(f"Goal succeeded. Distance travelled: {result.total_distance: 4.2f}")
        elif status == GoalStatus.STATUS_CANCELED:
            self.get_logger().info(f"Goal cancelled (preempted by new face pose)")
        elif status == GoalStatus.STATUS_ABORTED:
            self.get_logger().info(f"Goal aborted/pre-empted")
        else:
            self.get_logger().warn(f"Message from policy node: {result.message}")
